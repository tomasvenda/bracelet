...
static void (*raw_response_cb)(const uint8_t *, size_t);
void comms_set_raw_response_cb(void (*cb)(const uint8_t *, size_t)) { raw_response_cb = cb; }

static void on_mqtt_publish(struct mqtt_helper_buf topic, struct mqtt_helper_buf payload)
{
    if (raw_response_cb) {
        raw_response_cb((const uint8_t *)payload.ptr, payload.size);
        return;
    }
    char buf[64] = {0};
